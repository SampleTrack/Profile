...
@Client.on_message(filters.private & filters.text & filters.chat(AUTH_USERS) if AUTH_USERS else filters.private & filters.text)
...
async def pm_AutoFilter(client, msg, pmspoll=False):    
    if not pmspoll:
        ...
        if 2 < len(message.text) < 100:
            search = message.text
            searching_msg = await message.reply_text("🔍 Searching, please wait...", quote=True)
        ...
        searching_msg = await msg.message.reply_text("🔍 Searching, please wait...", quote=True)
    ...
    btn.insert(0, [InlineKeyboardButton("🔗 ʜᴏᴡ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅ 🔗", callback_data="howdl")])
    ...
    if imdb:
        try:
            cap = TEMPLATE.format(
                group=message.chat.title if hasattr(message.chat, 'title') else '',
            ...
            logger.exception(e)
            await searching_msg.edit_text(cap, reply_markup=InlineKeyboardMarkup(btn))
            await asyncio.sleep(IMDB_DELET_TIME)
            await searching_msg.delete()
    else:
