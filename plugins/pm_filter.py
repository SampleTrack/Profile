...
async def pm_spoll_choker(msg, txt):
    ...
    if not g_s:
        reply = await txt.edit_text("I Cᴏᴜʟᴅɴ'ᴛ Fɪɴᴅ Aɴʏ Mᴏᴠɪᴇ Iɴ Tʜᴀᴛ Nᴀᴍᴇ")
        await asyncio.sleep(10)
        await reply.delete()
        await msg.delete()
        return
    ...
    if not movielist:
        reply = await txt.edit_text("I Cᴏᴜʟᴅɴ'ᴛ Fɪɴᴅ Aɴʏᴛʜɪɴɢ Rᴇʟᴀᴛᴇᴅ Tᴏ Tʜᴀᴛ. Cʜᴇᴄᴋ Yᴏᴜʀ Sᴘᴇʟʟɪɴɢ")
        await asyncio.sleep(10)
        await reply.delete()
        await msg.delete()
        return
    ...
    btn = [[InlineKeyboardButton(text=movie.strip(), callback_data=f"pmspolling#{txt}#{user}#{k}")]
