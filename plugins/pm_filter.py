...
async def pm_AutoFilter(client, msg, txt, pmspoll=False):    
    if not pmspoll:
        ...
        if 2 < len(message.text) < 100:
            ...
            if not files: return await pm_spoll_choker(msg, txt)              
        else: return 
    else:
    ...
    if SHORT_URL and SHORT_API:          
        ...
        if SINGLE_BUTTON:
            ...
            btn = [[InlineKeyboardButton(text=f"{file.file_name}", callback_data=f'{pre}#{message.from_user.id}#{file.file_id}'),
    ...
    if offset != "":
        ...
        btn.append(
            ...
            InlineKeyboardButton(text="➡️ Next", callback_data=f"pmnext_{req}_{key}_{offset}")]
        )
    else:
        btn.append(
    ...
    try:
        if imdb and imdb.get('poster'):
            try:
                ...
                alt_photo = await message.reply_photo(photo=poster, caption=cap, reply_markup=InlineKeyboardMarkup(btn))
                await asyncio.sleep(IMDB_DELET_TIME)
                await alt_photo.delete()
        else:
            await wait_msg.edit_text(cap, reply_markup=InlineKeyboardMarkup(btn))
            await asyncio.sleep(IMDB_DELET_TIME)
            await wait_msg.delete()
    except Exception as e:
        logger.exception(e)
        fallback = await message.reply_text(cap, reply_markup=InlineKeyboardMarkup(btn))
        await asyncio.sleep(IMDB_DELET_TIME)
...
async def pm_spoll_choker(msg, txt):
    ...
    gs_parsed = []
    if not g_s:
        k = await txt.edit_text("I Cᴏᴜʟᴅɴ'ᴛ Fɪɴᴅ Aɴʏ Mᴏᴠɪᴇ Iɴ Tʜᴀᴛ Nᴀᴍᴇ", quote=True)
        await asyncio.sleep(10)
    ...
    if not movielist:
        k = await txt.edit_text("I Cᴏᴜʟᴅɴ'ᴛ Fɪɴᴅ Aɴʏᴛʜɪɴɢ Rᴇʟᴀᴛᴇᴅ Tᴏ Tʜᴀᴛ. Cʜᴇᴄᴋ Yᴏᴜʀ Sᴘᴇʟʟɪɴɢ", quote=True)
        await asyncio.sleep(10)
        return await k.delete()
    temp.PM_SPELL[str(msg.id)] = movielist
    btn = [[InlineKeyboardButton(text=movie.strip(), callback_data=f"pmspolling#{txt}#{user}#{k}")] for k, movie in enumerate(movielist)]
    btn.append([InlineKeyboardButton(text="Close", callback_data=f'pmspolling#{txt}#{user}#close_spellcheck')])
    await txt.edit_text("I Cᴏᴜʟᴅɴ'ᴛ Fɪɴᴅ Aɴʏᴛʜɪɴɢ Rᴇʟᴀᴛᴇᴅ Tᴏ Tʜᴀᴛ. Dɪᴅ Yᴏᴜ Mᴇᴀɴ Aɴʏ Oɴᴇ Oғ Tʜᴇsᴇ?", reply_markup=InlineKeyboardMarkup(btn), quote=True)
